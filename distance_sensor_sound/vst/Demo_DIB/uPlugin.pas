...
type
 ...
 public
  tmp_l, tmp_r, peak_l, peak_r, vol_l, vol_r: single;
  sampleCount: integer;
...
begin
 ...
 // inidivdually. In this example, the volume of the left and right
 ...
 begin
  ...
  // simple (and not very efficient) VU meter code:
  tmp_l := tmp_l + f_abs(outputs[0, i]);
  tmp_r := tmp_r + f_abs(outputs[1, i]);
  inc(sampleCount);
  if sampleCount >= 512 then
  begin
   sampleCount := 0;
   peak_l := 0.8 * peak_l + 0.2 * (tmp_l / 512);
   peak_r := 0.8 * peak_r + 0.2 * (tmp_r / 512);
   tmp_l := 0;
   tmp_r := 0;
  end;
 end;
