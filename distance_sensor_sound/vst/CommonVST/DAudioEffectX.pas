...
//      virtual void wantEvents (long filter = 1);     // default is 1 for this!
      ...
      function string2parameter(index: longint; text: pchar): boolean; virtual; // note: implies setParameter. text==0 is to be
      function getChannelParameter(channel, index: longint): Single; virtual;
      function getNumCategories: longint; virtual;
      function getProgramNameIndexed(category, index: longint; text: pchar): boolean; virtual;
      ...
      function canHostDo(text: pchar): longint; virtual;  // see 'hostCanDos' in audioeffectx.cpp
      ...
      function getEffectName(name: pchar): boolean; virtual;  // name max 32 char
      ...
      function canDo(text: pchar): longint; virtual; // see 'plugCanDos' in audioeffectx.cpp. return values:
      ...
      function getNextShellPlugin(name: pchar): longint; virtual;           // This opcode is only called, if Plugin is of type kPlugCategShell.
...
type
    THostCanDosArray = array[0..NumHostCanDos-1] of pchar;
    TPlugCanDosArray = array[0..NumPlugCanDos-1] of pchar;
...
begin
  ...
  case opcode of
    ...
    effShellGetNextPlugin:  v := getNextShellPlugin (pchar(ptr));
...
//------------------------------------------------------------------------------
function AudioEffectX.getHostVendorString(text: pchar): boolean;
begin
  if Assigned(audioMaster) then
...
//------------------------------------------------------------------------------
function AudioEffectX.getHostProductString(text: pchar): boolean;
begin
  if Assigned(audioMaster) then
...
//------------------------------------------------------------------------------
function AudioEffectX.canHostDo(text: pchar): longint;
begin
  Result := 0;
  if Assigned(audioMaster) then
...
function AudioEffectX.string2parameter(index: longint; text: pchar): boolean;
begin
  Result := FALSE;
end;
...
function AudioEffectX.getProgramNameIndexed(category, index: longint; text: pchar): boolean;
begin
  Result := FALSE;
end;
...
function AudioEffectX.getVendorString(text: pchar): boolean;
begin
  Result := FALSE;
end;

function AudioEffectX.getProductString(text: pchar): boolean;
begin
  Result := FALSE;
end;
...
function AudioEffectX.canDo(text: pchar): longint;
begin
  Result := 0;
end;
...
{$IFDEF VST_2_3_EXTENSIONS}
function AudioEffectX.getNextShellPlugin(name: pchar): longint;
begin
  Result := 0;
end;
{$ENDIF}
